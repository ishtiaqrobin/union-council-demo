...
import { X, RotateCcw, Building2, FileText, User, PenTool, Info, MapPin, Activity } from "lucide-react";
      ...
      <div className="p-5 flex flex-col gap-6 overflow-y-auto">
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider pb-1 border-b border-slate-200 dark:border-slate-800">
            <Building2 className="w-4 h-4" /> নিবন্ধকের কার্যালয় / ইউনিয়ন
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">ইউনিয়ন পরিষদ / কার্যালয়ের নাম</label>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider pb-1 border-b border-slate-200 dark:border-slate-800">
            <FileText className="w-4 h-4" /> নিবন্ধনের বিবরণ
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">মৃত্যু নিবন্ধন নং (১৭ ডিজিট)</label>
            <input type="text" value={data.registrationNoBn || data.meta.serial_no} onChange={(e) => {
              onChange({ ...data, registrationNoBn: e.target.value, meta: { ...data.meta, serial_no: e.target.value } });
            }} className={inputClass} />
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">নিবন্ধনের তারিখ</label>
              <input type="text" value={data.registrationDateBn || data.meta.issue_date} onChange={(e) => onChange({ ...data, registrationDateBn: e.target.value })} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">সনদ প্রদান তারিখ</label>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">মৃত্যুর তারিখ</label>
              <input type="text" value={data.dateOfDeathBn || ""} onChange={(e) => onChange({ ...data, dateOfDeathBn: e.target.value })} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">লিঙ্গ</label>
              <input type="text" value={data.genderBn || ""} onChange={(e) => onChange({ ...data, genderBn: e.target.value })} className={inputClass} />
            </div>
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">মৃত্যুর তারিখ (কথায়)</label>
            <input type="text" value={data.dateOfDeathInWordsBn || ""} onChange={(e) => onChange({ ...data, dateOfDeathInWordsBn: e.target.value })} className={inputClass} />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2 text-xs font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider pb-1 border-b border-slate-200 dark:border-slate-800">
            <MapPin className="w-4 h-4" /> ঠিকানা ও স্থান
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">স্থায়ী ঠিকানা</label>
            <input type="text" value={data.permanentAddressBn || ""} onChange={(e) => onChange({ ...data, permanentAddressBn: e.target.value })} className={inputClass} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">বর্তমান ঠিকানা</label>
            <input type="text" value={data.presentAddressBn || ""} onChange={(e) => onChange({ ...data, presentAddressBn: e.target.value })} className={inputClass} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">মৃত্যুর স্থান</label>
            <input type="text" value={data.placeOfDeathBn || ""} onChange={(e) => onChange({ ...data, placeOfDeathBn: e.target.value })} className={inputClass} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-amber-600 dark:text-amber-400">মৃত্যুর কারণ</label>
            <input type="text" value={data.causeOfDeathBn || ""} onChange={(e) => onChange({ ...data, causeOfDeathBn: e.target.value })} className={inputClass} />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider pb-1 border-b border-slate-200 dark:border-slate-800">
            <Info className="w-4 h-4" /> স্বাক্ষরকারী তথ্য
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">বাম স্বাক্ষরকারী (প্রশাসনিক কর্মকর্তা)</label>
              <input type="text" value={data.leftSignatoryNameBn || ""} onChange={(e) => onChange({ ...data, leftSignatoryNameBn: e.target.value })} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">ডান স্বাক্ষরকারী (চেয়ারম্যান/প্যানেল)</label>
