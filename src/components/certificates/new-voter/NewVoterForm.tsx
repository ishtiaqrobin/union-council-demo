        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-none">
              {isEn ? "New Voter Certificate Editor" : "নতুন ভোটার প্রত্যয়ন লাইভ এডিটর"}
            </h3>
            <span className="text-[11px] text-slate-500 dark:text-slate-400">
              {isEn ? "Create New Voter Certificate" : "নতুন ভোটার প্রত্যয়ন তৈরি করুন"}
            </span>
          </div>
        </div>
      ...
      <div className="p-5 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 p-3 bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
          ...
          <div className="grid grid-cols-1 gap-1.5 mt-1">
            ...
            >
              {isEn ? "1. Standard New Voter Registration" : "১. আদর্শ নতুন ভোটার নিবন্ধন প্রত্যয়ন"}
            </button>
            <button
              onClick={() => applyTemplate(
              ...
              className="text-[11px] text-left p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-emerald-500 transition-colors font-semibold"
            >
              {isEn ? "2. Expatriate New Voter" : "২. প্রবাসে থাকার কারণে নতুন ভোটার"}
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                {isEn ? "Birth Reg. No (17 Digits)" : "জন্ম নিবন্ধন নং (১৭ ডিজিট)"}
              </label>
              <input
                type="text"
                value={getApplicantValue("birth_reg_no")}
                onChange={(e) => updateApplicant("birth_reg_no", e.target.value)}
                className={inputClass}
                placeholder="20068217631012345"
              />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                {isEn ? "Date of Birth (DOB)" : "জন্ম তারিখ (DOB)"}
              </label>
              <input
                type="text"
                value={getApplicantValue("dob")}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              placeholder={isEn ? "Enter custom description text..." : "এখানে নতুন ভোটার প্রত্যয়ন বিবরণ লিখুন..."}
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">
