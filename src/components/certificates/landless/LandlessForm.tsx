        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-none">
              {isEn ? "Landless Certificate Form" : "ভূমিহীন প্রত্যয়ন তথ্য"}
            </h3>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-cyan-600 dark:text-cyan-400">
              {isEn ? "Landless Info Details" : "ভূমিহীন তথ্য বিবরণ"}
            </label>
            <textarea
              value={isEn ? (data.landDetailsEn || data.landDetailsBn || "") : (data.landDetailsBn || "")}
              onChange={(e) => {
                const key = isEn ? "landDetailsEn" : "landDetailsBn";
                onChange({ ...data, [key]: e.target.value });
              }}
              className={`${inputClass} h-24`}
              placeholder="আমার জানামতে তার কোন জায়গা জমি নাই। তিনি একজন ভূমিহীন।"
            />
          </div>
        </div>
