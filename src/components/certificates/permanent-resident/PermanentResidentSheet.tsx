...
interface PermanentResidentSheetProps {
  data: CertificateData;
  lang?: "bn" | "en";
}

export function PermanentResidentSheet({ data, lang = "bn" }: PermanentResidentSheetProps) {
  const { union, meta, applicant, signatory, customDescriptionBn, customDescriptionEn, closingWishBn, closingWishEn } = data;
    ...
    >
      ...
      <div className="flex justify-between items-center text-xs text-black font-sans px-1 pb-1">
        ...
        <div className={`text-black text-sm ${isEn ? "font-serif" : "font-solaiman"}`}>
          {isEn ? (meta.cert_title_en || "Permanent Resident Certificate") : (meta.cert_title || "স্থায়ী বাসিন্দা সনদপত্র")}
        </div>
        <div className="w-[180px]" />
      </div>

      {/* Main Certificate Box with Outer Padding and Thick Gradient Border */}
      <div className="flex-1 relative p-4 my-8 mx-12 bg-gradient-to-br from-teal-600 via-emerald-500 to-teal-600 shadow-md">
        <div className="certificate-inner-frame w-full h-full bg-white pt-5 px-12 pb-4 relative flex flex-col justify-between z-10">
          ...
          <div className="cert-meta-ribbon flex justify-between items-center mt-[10px] px-1">
            ...
            <div className="meta-badge-container flex justify-center flex-1">
              <div className="cert-badge bg-teal-700 text-white text-xl font-semibold px-8 py-1 rounded-[4px] tracking-wide inline-block shadow-sm">
                {isEn ? (meta.cert_title_en || "Permanent Resident Certificate") : meta.cert_title}
              </div>
            </div>
          ...
          <div className="cert-content-body mt-6 px-2 flex-1 flex flex-col justify-start">
            {isEn ? (
              <p className="cert-paragraph text-base leading-[2.2] text-black text-justify font-serif">
                ...
                (NID/Birth Reg No:{" "}
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
                  {applicant.nid_no_en || applicant.nid_no || applicant.birth_reg_no_en || applicant.birth_reg_no || "-"}
                </span>
                ), {applicant.spouse_name_en || applicant.spouse_name ? "Spouse:" : "Father:"}{" "}
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
                  {applicant.spouse_name_en || applicant.spouse_name || applicant.father_name_en || applicant.father_name}
                </span>
                , Mother:{" "}
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
                ...
                .{customDescriptionEn || customDescriptionBn ? ` ${customDescriptionEn || customDescriptionBn}` : ""}
              </p>
            ) : (
              <p className="cert-paragraph text-base leading-[2.2] text-black text-justify font-solaiman">
                ...
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
                  {applicant.nid_no || applicant.birth_reg_no || "-"}
                </span>
                ), {applicant.spouse_name ? `স্বামী:` : `পিতা:`}{" "}
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
                  {applicant.spouse_name || applicant.father_name}
                </span>
                , মাতাঃ{" "}
                <span className="font-semibold border-b border-dotted border-black pb-[1px]">
            ...
            {(closingWishEn || closingWishBn) && (
              <p className={`cert-closing text-base font-semibold text-black mt-3 ${isEn ? "font-serif" : "font-solaiman"}`}>
                {isEn ? (closingWishEn || closingWishBn) : closingWishBn}
              </p>
            )}
          </div>
