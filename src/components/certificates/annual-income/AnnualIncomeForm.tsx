...
import { X, RotateCcw, Building2, FileText, User, MapPin, PenTool, DollarSign, Info } from "lucide-react";
...
export function AnnualIncomeForm({
  data,
  onChange,
  onReset,
  isOpen,
  onClose,
}: AnnualIncomeFormProps) {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
              <DollarSign className="w-3.5 h-3.5" /> বার্ষিক আয়ের পরিমাণ
            </label>
            <input type="text" value={data.incomeAmountBn || ""} onChange={(e) => onChange({ ...data, incomeAmountBn: e.target.value })} className="px-3 py-2 bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-bold text-emerald-600 dark:text-emerald-300" />
          </div>
        </div>
