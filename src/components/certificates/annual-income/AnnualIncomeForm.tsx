...
import { X, RotateCcw, Building2, FileText, User, PenTool, DollarSign, Info, Globe } from "lucide-react";
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-none">
              {isEn ? "Annual Income Certificate Form" : "বার্ষিক আয় প্রত্যয়ন তথ্য"}
            </h3>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
              <DollarSign className="w-3.5 h-3.5" /> {isEn ? "Annual Income Amount" : "বার্ষিক আয়ের পরিমাণ"}
            </label>
            <input
              type="text"
              value={isEn ? (data.incomeAmountEn || data.incomeAmountBn || "") : (data.incomeAmountBn || "")}
              onChange={(e) => {
                const key = isEn ? "incomeAmountEn" : "incomeAmountBn";
