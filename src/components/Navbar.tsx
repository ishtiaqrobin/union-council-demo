...
const ICON_MAP: Record<string, React.ReactNode> = {
  ...
  UserCheck2: <UserCheck2 className="w-4 h-4 text-indigo-400" />,
  ShieldCheck: <ShieldCheck className="w-4 h-4 text-teal-400" />,
  Home: <Home className="w-4 h-4 text-purple-400" />,
  FileCheck: <FileCheck className="w-4 h-4 text-rose-400" />,
  HeartHandshake: <HeartHandshake className="w-4 h-4 text-amber-400" />,
  Briefcase: <Briefcase className="w-4 h-4 text-sky-400" />,
  Award: <Award className="w-4 h-4 text-slate-300" />,
  Banknote: <Banknote className="w-4 h-4 text-emerald-300" />,
  MapPinOff: <MapPinOff className="w-4 h-4 text-cyan-400" />
};
      ...
      <div className="max-w-[1700px] mx-auto px-4 sm:px-6 py-2.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="text-xs font-semibold text-emerald-400 tracking-wider font-siliguri flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-amber-400" /> ই-সনদ প্ল্যাটফর্ম
              </span>
              <span className="text-base font-bold text-slate-100 font-siliguri leading-tight">
        ...
        <div className="relative flex-1 max-w-xl">
          <button
            ...
            className="w-full flex items-center justify-between gap-2 px-4 py-2 rounded-xl bg-slate-800/90 border border-slate-700/80 text-slate-200 hover:bg-slate-800 hover:border-slate-600 transition-all text-sm font-medium font-siliguri shadow-inner"
          >
            <div className="flex items-center gap-2.5 truncate">
              <FileText className="w-4 h-4 text-emerald-400 shrink-0" />
              <span className="text-slate-400 text-xs hidden sm:inline">সনদ নির্বাচন:</span>
          ...
          {dropdownOpen && (
            <>
              ...
              <div className="absolute left-0 right-0 top-full mt-2 z-[1200] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="p-3 bg-slate-950/80 border-b border-slate-800 flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent text-sm text-slate-100 placeholder-slate-500 focus:outline-none font-siliguri"
                    autoFocus
                  />
                  {searchQuery && (
                    <button
                ...
                <div className="max-h-[380px] overflow-y-auto p-2 grid grid-cols-1 md:grid-cols-2 gap-1">
                  {filteredMenuItems.length > 0 ? (
                        ...
                        >
                          ...
                          <div className="flex flex-col min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              {isSelected && (
                                <span className="text-[10px] font-bold bg-emerald-500 text-slate-950 px-1.5 py-0.2 rounded">
                                  সক্রিয়
                                </span>
                              )}
                            </div>
                    ...
                    <div className="col-span-2 p-6 text-center text-slate-400 text-sm font-siliguri">
                ...
                {/* Footer status */}
                <div className="px-4 py-2 bg-slate-950 text-xs text-slate-400 font-siliguri flex justify-between items-center border-t border-slate-800">
                  <span>সব টেম্পলেটে লাইভ এডিটর যুক্ত রয়েছে</span>
                  <span className="text-emerald-400 font-medium">100% Dynamic</span>
                </div>
              </div>
            </>
          )}
